 ...
 */
 if ( ! function_exists( 'et_builder_include_regions_option' ) ) :
	function et_builder_include_regions_option( $args = array() ) {
		...
		$output = "\t" . "<% var et_pb_include_regions_temp = typeof data !== 'undefined' && typeof data.et_pb_include_regions !== 'undefined' ? data.et_pb_include_regions.split( ',' ) : []; et_pb_include_regions_temp = typeof data === 'undefined' && typeof et_pb_include_regions !== 'undefined' ? et_pb_include_regions.split( ',' ) : et_pb_include_regions_temp; %>" . "\n";
	
		// if ( $args['use_terms'] ) {
			$cats_array = get_terms( $args['term_name'] );
		// } else {
		// 	$cats_array = get_regions( apply_filters( 'et_builder_get_regions_args', 'hide_empty=0' ) );
		// }
		...
		foreach ( $cats_array as $category ) {
			$contains = sprintf(
				'<%%= _.contains( et_pb_include_regions_temp, "%1$s" ) ? checked="checked" : "" %%>',
				esc_html( $category->term_id )
			);
	
			$output .= sprintf(
				'%4$s<label><input type="checkbox" name="et_pb_include_regions" value="%1$s"%3$s> %2$s</label><br/>',
		...
		$output = '<div id="et_pb_include_regions">' . $output . '</div>';
...
if ( ! function_exists( 'et_builder_include_provinces_option' ) ) :
	function et_builder_include_provinces_option( $args = array() ) {
		...
		$output = "\t" . "<% var et_pb_include_provinces_temp = typeof data !== 'undefined' && typeof data.et_pb_include_provinces !== 'undefined' ? data.et_pb_include_provinces.split( ',' ) : []; et_pb_include_provinces_temp = typeof data === 'undefined' && typeof et_pb_include_provinces !== 'undefined' ? et_pb_include_provinces.split( ',' ) : et_pb_include_provinces_temp; %>" . "\n";
	
		// if ( $args['use_terms'] ) {
			$cats_array = get_terms( $args['term_name'] );
		// } else {
		// 	$cats_array = get_provinces( apply_filters( 'et_builder_get_provinces_args', 'hide_empty=0' ) );
		// }
		...
		foreach ( $cats_array as $category ) {
			$contains = sprintf(
				'<%%= _.contains( et_pb_include_provinces_temp, "%1$s" ) ? checked="checked" : "" %%>',
				esc_html( $category->term_id )
			);
	
			$output .= sprintf(
				'%4$s<label><input type="checkbox" name="et_pb_include_provinces" value="%1$s"%3$s> %2$s</label><br/>',
				esc_attr( $category->term_id ),
				esc_html( $category->name ),
				$contains,
				"\n\t\t\t\t\t"
		...
		$output = '<div id="et_pb_include_provinces">' . $output . '</div>';
			...
			<div class='et_divi_builder et-bfb-page-preloading{$additional_bfb_class}'>
				<script>
					...
					document.addEventListener('DOMContentLoaded', function() {
						var outer = jQuery('<div>', {id: 'et-boc', className: 'et-boc'});
		...
		if ( $is_switch_to_classic_allowed ) {
			echo '<div class="et-bfb-optin-cta">';
				echo '<p class="et-bfb-optin-cta__message et-bfb-optin-cta__message--success">';
					...
					echo '<a href="' . esc_url( $classic_builder_url ) . '" class="et-bfb-optin-cta__button">';
						echo esc_html__( 'Switch Back To Classic Divi Builder', 'et_builder' );
					echo '</a>';
				echo '</p>';
			echo '</div>';
		}
...
function et_fb_get_builder_shortcode_object( $post_type, $post_id, $layout_type ) {
	 ...
	 */
	$post_content = apply_filters( 'et_fb_load_raw_post_content', et_fb_maybe_get_bfb_initial_content( $post_content, $post_id ) );
