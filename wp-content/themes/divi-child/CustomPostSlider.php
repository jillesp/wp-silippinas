...
class ET_Builder_Module_Post_Slider_Custom extends ET_Builder_Module_Type_PostBased {
	function init() {
	...
	public function get_transition_fields_css_props() {
		...
		$fields['dot_nav_custom_color'] = array( 'background-color' => et_pb_slider_custom_options()->get_dots_selector() );
		$fields['arrows_custom_color']  = array( 'all' => et_pb_slider_custom_options()->get_arrows_selector() );
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		$this->generate_responsive_hover_style( 'arrows_custom_color', et_pb_slider_custom_options()->get_arrows_selector(), 'color' );
		$this->generate_responsive_hover_style( 'dot_nav_custom_color', et_pb_slider_custom_options()->get_dots_selector(), 'background-color' );
		...
		if ( $query->have_posts() ) {
			...
			?>
			<div class="et_pb_slide et_pb_media_alignment_center<?php echo esc_attr( $slide_class ); ?>" <?php if ( 'on' !== $parallax && 'off' !== $show_image && 'background' === $image_placement ) { printf( 'style="background-image:url(%1$s)"', esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ) );  } ?>>
				...
				<div class="et_pb_container clearfix">
					<div class="et_pb_slider_custom_container_inner">
		...
		$this->add_classname( array(
			'et_pb_slider_custom',
			"et_pb_post_slider_custom_image_{$image_placement}",
		) );

		$this->add_classname( 'et_pb_slider_custom_fullwidth_off' );

		if ( 'off' === $show_arrows ) {
			$this->add_classname( 'et_pb_slider_custom_no_arrows' );
		}

		if ( 'off' === $show_pagination ) {
			$this->add_classname( 'et_pb_slider_custom_no_pagination' );
		}

		if ( 'on' === $parallax ) {
			$this->add_classname( 'et_pb_slider_custom_parallax' );
		}

		if ( 'on' === $auto ) {
			$this->add_classname( array(
				'et_slider_custom_auto',
				"et_slider_custom_speed_{$auto_speed}",
			) );
		}

		if ( 'on' === $auto_ignore_hover ) {
			$this->add_classname( 'et_slider_custom_auto_ignore_hover' );
		}

		if ( 'on' === $show_image_video_mobile ) {
			$this->add_classname( 'et_pb_slider_custom_show_image' );
		}

		if ( 'on' === $use_bg_overlay ) {
			$this->add_classname( 'et_pb_slider_custom_with_overlay' );
		}

		if ( 'on' === $use_text_overlay ) {
			$this->add_classname( 'et_pb_slider_custom_with_text_overlay' );
		}
